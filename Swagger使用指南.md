...
1. 找到 **科室管理** 分组
2. 点击 `POST /department/batch` - 批量创建科室
3. 输入请求体：
   ```json
   {
     "departments": [
...
3. 输入请求体：
   ```json
   {
     ...
     "clinics": [
       {
         "name": "普通门诊",
         "description": "周一至周五 8:00-17:00"
       },
       {
         "name": "专家门诊",
         "description": "周一、周三、周五 8:00-12:00"
       },
       {
         "name": "特需门诊",
         "description": "预约制，需提前3天预约"
       }
     ]
   }
   ```
...
**注意**：
- 批量创建门诊时，必须指定有效的 `departmentId`（科室ID）
- 所有创建的门诊都会归属于指定的科室
- 批量操作使用事务，如果有任何一个失败，所有操作都会回滚
